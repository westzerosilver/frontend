...
const MealEnd = () => {
    ...
    const [startEatingTime, setStartEatingTime] = useState('');
    const [endEatingTime, setEndEatingTime] = useState('');
    const navigate = useNavigate();

    useEffect(() => {
        // 로컬 스토리지에서 날짜와 시간 가져오기
        const storedDate = localStorage.getItem('selectedDate');
        const storedStartEatingTime = localStorage.getItem('startEatingTime');
        const storedEndEatingTime = localStorage.getItem('endEatingTime');
        ...
        if (storedStartEatingTime) {
            setStartEatingTime(moment(storedStartEatingTime).format('HH:mm'));
        }

        if (storedEndEatingTime) {
            setEndEatingTime(moment(storedEndEatingTime).format('HH:mm'));
        }
